...
<body class='hold-transition skin-green-light fixed <?= $sidebar ?>'>
	...
	<div class='wrapper'>
		<header class='main-header'>
			<a href='?' class='logo' style='background-color:#283c86'>
				...
				<div class='navbar-custom-menu'>
					<ul class='nav navbar-nav'>
						<?php if ($pengawas['level'] == 'admin') : ?>
							<li><a style="color:#fff" href='?pg=pengumuman'><i class="fas fa-bullhorn fa-lg  "></i></a></li>
							<li class='dropdown notifications-menu'>
								<a href='#' class='dropdown-toggle' data-toggle='dropdown' style="color:#fff">
									<i class="fas fa-desktop fa-lg fa-fw"></i> <span><?= strtoupper($setting['server']) ?></span>
								</a>
								<ul class="dropdown-menu" style="height:80px">
									<li class="header">Ganti Server</li>
									<li>
										...
										<ul class="menu">
											<?php if ($setting['server'] == 'lokal') { ?>
												<li>
													<a id="btnserver" href="#">
														<i class="fa fa-users text-aqua"></i> Pusat
													</a>
												</li>
											<?php } else { ?>
												<li>
													<a id="btnserver" href="#">
														<i class="fa fa-users text-aqua"></i> Lokal
													</a>
												</li>
											<?php } ?>
										</ul>
		...
		<aside class='main-sidebar' style="background-color: #fff">
			...
			<section class='sidebar'>
				...
				<ul class=' sidebar-menu tree data-widget=' tree>
					...
					<hr style="margin:0px">
					<?php
					if ($setting['jenjang'] == 'MA') {
						$jenjang = 'MA/MAK';
					} elseif ($setting['jenjang'] == 'MTS') {
						$jenjang = 'MTS';
					} else {
						$jenjang = 'MI';
					}
				...
				<?php elseif ($pg == 'uplfotosiswa') : ?>
					...
					<div class='box box-solid'>
						<div class='box-header with-border'>
							...
							<div class='box-tools pull-right '>
								<form action='' method='post'>
									<button class='btn btn-sm bg-maroon' name='hapussemuasfoto'>hapus semua foto</button>
								</form>
							</div>
						</div><!-- /.box-header -->
						<div class='box-body'>
				...
				<?php elseif ($pg == 'pk') : ?>
					...
					<?php else : ?>
						<div class="panel panel-body panel-info">
							<h4>
								Untuk Jenjang MI  sampai MTS , tidak memiliki jurusan
							</h4>
						</div>
					<?php endif; ?>
							...
							<div class='col-md-3'>
								<div class='box box-solid'>
									<div class='box-body box-profile'>
										<img class='profile-user-img img-responsive img-circle' alt='User profile picture' src='<?= $homeurl ?>/dist/img/logokemenag.png'>
						...
						<div class='row'>
							<div class='col-md-12'>
								<div class='alert alert-warning alert-dismissible'>
									<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
									<i class='icon fa fa-info'></i>
									Status peserta akan muncul saat ujian berlangsung coy..
								</div>
								<div class='box box-solid'>
									<div class='box-header with-border'>
										<h3 class='box-title'>Status Peserta </h3>
		...
		<footer class='main-footer hidden-xs'>
			<div class='container'>
				<div class='pull-left hidden-xs'>
					<strong>
						<span id='end-sidebar'>
							&copy; 2022 <?= APLIKASI . " v" . VERSI  ?> - by <a href="https://aplikasipendidikan.net">Aplikasi Pendidikan</a>
						</span>
					</strong>
				</div>
	...
	<script>
		...
		<?php if ($pg == 'status') { ?>
			var autoRefresh = setInterval(
				function() {
					$('#divstatus').load("statuspeserta.php?id=<?= $_GET['id'] ?>&idu=<?= $_GET['idu'] ?>");
				}, 5000
			);
		<?php } ?>
		<?php if ($pg == 'statusall') { ?>
			var autoRefresh = setInterval(
				function() {
					$('#divstatusall').load("statusall.php?id=76310EEFF2B5D3C887F238976A421B638CFEB0942AB8249CD0A29B125C91B3E5");
				}, 5000
			);
		<?php } ?>
		<?php if ($pg == 'statussiswa') { ?>
			var autoRefresh = setInterval(
				function() {
					$('#divstatussiswa').load("statussiswa.php?id=76310EEFF2B5D3C887F238976A421B638CFEB0942AB8249CD0A29B125C91B3E5");
				}, 5000
			);
		<?php } ?>
		var autoRefresh = setInterval(
